...
from object_detect.srv import Customer_Interest, Customer_InterestResponse


class Customer_Interest_Node:
    def __init__(self) -> None:
        ...
        self.pose_detector = mp.tasks.vision.PoseLandmarker.create_from_options(
            mp.tasks.vision.PoseLandmarkerOptions(
                base_options=mp.tasks.BaseOptions(
                    model_asset_path=rospkg.RosPack().get_path('object_detect') + "/model/pose_landmarker_heavy.task"),
        ...
        self.face_detector = mp.tasks.vision.FaceLandmarker.create_from_options(
            mp.tasks.vision.FaceLandmarkerOptions(
                base_options=mp.tasks.BaseOptions(
                    model_asset_path=rospkg.RosPack().get_path('object_detect') + "/model/face_landmarker.task"),
                running_mode=mp.tasks.vision.RunningMode.IMAGE,
                num_faces=2))
        ...
        self.server = rospy.Service(
            '/restaurant/customer_interest', Customer_Interest, self.process)

    def process(self, req):
        begin = rospy.get_time()
        customer_interest = False

        while (rospy.get_time() - begin < 10 and customer_interest == False):
            if self.image_update:
                self.image_update = False
                mp_image = mp.Image(
                    image_format=mp.ImageFormat.SRGB, data=self.image)
                ...
                for idx in range(len(pose_result.pose_landmarks)):
                    ...
                    if self.determine_interest(self.image, pose_landmarks):
                        customer_interest = True
                        print("Customer Call")
                        break

        return Customer_InterestResponse(customer_interest)

    def image_callback(self, image_msg):
        self.image = self.bridge.imgmsg_to_cv2(
            image_msg, desired_encoding='passthrough')
        self.img_height, self.img_width = self.image.shape[0], self.image.shape[1]
        self.image_update = True

    def determine_interest(self, image, pose_landmarks, face_landmarks=0) -> bool:

        _, nose_y = self.get_landmark_position(
            pose_landmarks, mp.solutions.pose.PoseLandmark.NOSE)
        ...
        right_hand_up = True if right_wrist_y < nose_y - 20 else False
        ...
        left_hand_up = True if left_wrist_y < nose_y - 20 else False

        # face_me = self.if_face_me(image, face_landmarks)

        return right_hand_up or left_hand_up
...
if __name__ == '__main__':
    rospy.init_node('customer_interest_node')
    ci = Customer_Interest_Node()
    rospy.spin()
