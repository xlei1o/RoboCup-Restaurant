...
class Grasp:
    ...
    def get_absolute_pose(self,msg):
        self.object_relative_pose = msg
        
        try:
            # connect to the rel2abs service
            self.rel2abs_pose = rospy.ServiceProxy('grasp/rel2abs', Rel2Abs)
            msg = PostStamped(msg)
            msg.header.frame_id = "xtion_rgb_frame" # ToDO
            self.object_absolute_pose=self.rel2abs_pose(msg).absolutepose.pose
...
if __name__ == "__main__":
    ...
    pre_object_service = rospy.Service(
        "grasp//pre_object", Pre_object, Grasp.preparation)
    
    grasp_object_service = rospy.Service(
        "grasp/grasp_object", Empty, Grasp.pick)
    
    place_object_service = rospy.Service(
        "grasp/place_object", Empty, Grasp.place)
